...
async function buildMorningBriefEmbed() {
  ...
  // Internal server-to-server requests need an API key (no Origin header → gateway rejects)
  const internalKey = (process.env.WORLDMONITOR_VALID_KEYS || '').split(',').filter(Boolean)[0] || '';
  const internalHeaders = {
    'Content-Type': 'application/json',
    ...(internalKey ? { 'X-WorldMonitor-Key': internalKey } : {}),
  };

  console.log('[Brief] Fetching data from', base, internalKey ? '(with API key)' : '(no API key)');
  const [riskResult, newsResult] = await Promise.allSettled([
    fetch(`${base}/api/intelligence/v1/get-risk-scores?region=global`, { headers: internalHeaders }).then(r => r.json()),
    fetch(`${base}/api/news/v1/list-feed-digest?variant=full&lang=en`, {
      headers: internalHeaders,
    }).then(r => r.json()),
  ]);

  console.log('[Brief] riskResult:', riskResult.status, riskResult.status === 'rejected' ? riskResult.reason?.message : `ciiScores=${riskResult.value?.ciiScores?.length}`);
  console.log('[Brief] newsResult:', newsResult.status, newsResult.status === 'rejected' ? newsResult.reason?.message : `keys=${newsResult.value ? Object.keys(newsResult.value).join(',') : 'null'}`, newsResult.value?.error || '');
  ...
  if (newsResult.status === 'fulfilled' && newsResult.value?.categories) {
    ...
    // publishedAt from the API is milliseconds; formatAgo expects seconds
    const toSec = ts => (ts > 1e11 ? Math.floor(ts / 1000) : ts);

    // Helper: deduplicate by title across merged category arrays
    ...
    console.log('[Brief] API categories:', Object.keys(cats).join(', '));

    // ── Technology sector ──────────────────────────────────────────────────
    const techItems = dedup([
    ...
    if (techItems.length) {
      const lines = techItems.map(item => {
        const ago = formatAgo(toSec(item.publishedAt));
    ...
    // No alert filter — show top recent news from geo categories
    const geoItems = dedup([
...
async function triggerMorningBrief() {
  ...
  if (process.env.DISCORD_WEBHOOK_URL) {
    try {
      ...
      console.error('[Brief] triggerMorningBrief error:', err.message, err.stack);
    }
  }
}
